...
void CTileManager::UpdateTypes(HWND hWnd)
{
    ...
    for (int idx = 0; idx < nTileCount; ++idx)
    {
        ...
        if (STDHelpers::Contains(tile, "road", true) || STDHelpers::Contains(tile, "highway", true))
         ...
         if (STDHelpers::Contains(tile, "crystal", true) || STDHelpers::Contains(tile, "blue", true) ||
            STDHelpers::Contains(tile, "swamp", true))
            { CTileManager::Datas[Nodes_FS].push_back(idx); other = false; }
       ...
       if (STDHelpers::Contains(tile, "tunnel", true))
        ...
        if (STDHelpers::Contains(tile, "ice", true))
            { CTileManager::Datas[Nodes_Ice].push_back(idx); other = false; }
