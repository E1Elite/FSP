...
{
	RunTime::ResetMemoryContentAt(0x585E58 + 0x4, &RunTime::Messages::EDIT_KILLFOCUS, 4); // name update //fs
	RunTime::ResetMemoryContentAt(0x585EA0 + 0x4, &RunTime::Messages::COMBOBOX_KILLFOCUS, 4); // param update //fs

	auto addr = &CScriptTypesExt::PreTranslateMessageExt;
	RunTime::ResetMemoryContentAt(0x585FD8, &addr, 4); //fs
	auto addr2 = &CScriptTypesExt::OnInitDialogExt;
	RunTime::ResetMemoryContentAt(0x586004, &addr2, 4); //fs
}
...
void CScriptTypesExt::UpdateParams(int actionIndex)
{
	...
	case 8:
		...
		break;
	case 9:
		CScriptTypesFunctions::CScriptTypes_LoadParams_Speeches(this->CCBScriptParameter);
		break;
	case 10:
...
BOOL CScriptTypesExt::OnInitDialogExt()
{
	...
	// Initialize defaults
	const char** pNames = reinterpret_cast<const char**>(0x5BF2D8); //fs
	const char** pDescriptions = reinterpret_cast<const char**>(0x5BF3C4); //fs
	...
	if (auto entities = fadata.GetSection("Scripts"))
	{
		char* pParseBuffer[5];
		for (auto& pair : entities->EntitiesDictionary)
		{
